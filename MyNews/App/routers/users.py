...
async def admin_update_user_role(
    ...
    # 关键保护：管理员不能把自己降级，避免系统失去管理员入口。
    if current_admin_user.id == user_id and payload.role != "admin":
...
async def admin_update_user_status(
    ...
    # 关键保护：管理员不能禁用自己，避免后台被彻底锁死。
    if current_admin_user.id == user_id and payload.status != "active":
